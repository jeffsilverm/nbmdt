...
class Network(Layer):
    ...
    def __init__(self, family_):
        ...
        self.route_dict = rt  # dgw is Default GateWay
...
jeffs@jeffs-desktop:/home/jeffs/python/nbmdt  (dev_0) *  $ 
        ...
        for line in lines:  # lines is the output of the ip route list command
            ...
            if fields[1] == "via":
                ...
                assert fields[
                           3] == 'dev', f"since fields[1] is 'via', fields[3] should be 'dev' but is actually " \
                                        f"{fields[3]}."
                route["dev"] = fields[4]
                field_ctr = 5
            elif fields[1] == "dev":
...
>>> 
            ...
            try:
                ...
                this_address: str = socket.getaddrinfo(address, port=None, family=self.family)[0][4][0]
            except (ValueError, socket.gaierror) as ve:
                print(
                    f"Tried to convert {address} to address using family_ {self.family} {ve}.  Trying again with IPv6",
                    file=sys.stderr)
                # If this doesn't work, then this address is hopeless.  It will raise an exception
                # Note that getaddrinfo returns a list of tuples.  We want the first element of that list,
                # a tuple.  The 4th element
                # of that tuple is another tuple of which the first element is the IPv6 address, a string. (What are
                # the other elements?)
                this_address: str = socket.getaddrinfo(address, port=None, family=socket.AF_INET6)[0][4][0]
                print(f"IPv6 worked.", file=sys.stderr)
            address: str = this_address[0][4][0]
        ...
        try:
            if self.family == socket.AF_INET6:
                ...
                try:
                    ...
                    # True
                    # >>> ipaddress.IPv6Address(ipaddress.IPv6Address('2001:500:88:200::8'))  in
                ...
                if not is_gua:
                    ...
                    dev = None  # This is a stopgap measure.  The ping command will fail.
                else:
                    ...
                    # To create the case, connect 2 devices to the default gateway (DGW), for example a wired
                    # ethernet and
    ...
    for family in [socket.AF_INET, socket.AF_INET6]:
        ...
        for t in test_host_list:
            ...
            try:
                ...
                print(f"{t} could not be pinged due to {str(g)}. Family_str is {routing_table.family_str}  Continuing",
                      file=sys.stderr)
    print(40 * "-")
